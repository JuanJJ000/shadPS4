...
struct GPUSettings {
    ...
    Setting<u32> readback_work_submit_budget{0};
    ...
    std::vector<OverrideItem> GetOverrideableFields() const {
        return std::vector<OverrideItem>{
            ...
            make_override<GPUSettings>("readback_work_submit_budget",
                                       &GPUSettings::readback_work_submit_budget),
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(GPUSettings, window_width, window_height, internal_screen_width,
                                   ...
                                   readbacks_mode, readback_work_submit_budget,
...
#define SETTING_FORWARD_BOOL_READONLY(group, Name, field)                                          \
    ...
    SETTING_FORWARD(m_gpu, ReadbackWorkSubmitBudget, readback_work_submit_budget)
